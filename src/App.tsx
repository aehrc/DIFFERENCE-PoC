...
function App() {
  ...
  const { stylesheetUrl } = config.branding || {};

  // dynamically load stylesheet
  useEffect(() => {
    if (!stylesheetUrl) return;

    import(/* @vite-ignore */ stylesheetUrl).catch(console.error);
  }, [stylesheetUrl]);
  ...
  return (
    <TooltipProvider delayDuration={100}>
      <SnackbarProvider maxSnack={1} action={CloseSnackbar}>
        <SourceFhirServerContextProvider fhirServerUrl={config.fhirServerUrl}>
